...
/*   Updated: 2020/02/27 03:18:50 by abassibe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/malloc.h"

static void	*realloc_large(t_header *data, t_meta_data *meta_data,
...
static void	*realloc_tiny_small(t_header *data, t_meta_data *meta_data,
		t_header *preview, size_t size)
{
	void	*ret;

	if ((data->type == SMALL && size <= MAX_SMALL_SIZE) ||
			(data->type == TINY && size <= MAX_TINY_SIZE))
		return (meta_data->addr);
	ret = malloc(size);
